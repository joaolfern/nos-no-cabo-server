...
class PreWebsiteSchema(BaseModel):
    ...
    def url_must_be_valid(cls, v):
        if not v or not v.strip():
            abort(
                make_response(
                    jsonify(ErrorSchema(message="URL é obrigatória.").dict()), 400
                )
            )
        if not v.startswith("http"):
            abort(
                make_response(
                    jsonify(ErrorSchema(message="A URL deve começar com http.").dict()),
                    400,
                )
            )
        return v
...
class PreWebsiteUpdateBodySchema(PreWebsiteResponseSchema):
    keywords: list[str] = []
